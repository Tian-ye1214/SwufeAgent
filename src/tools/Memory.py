...
def _message_has_user_prompt(msg: Any) -> bool:
    return isinstance(msg, ModelRequest) and any(
        isinstance(p, UserPromptPart) for p in msg.parts
    )


def _turn_has_agent_content(messages: list) -> bool:
    for msg in messages:
...
def _split_messages_into_turns(messages: list) -> list[list]:
    ...
    n = len(messages)
    while i < n:
        if not _message_has_user_prompt(messages[i]):
            i += 1
            continue
        start = i
        i += 1
        while i < n and not _message_has_user_prompt(messages[i]):
            i += 1
        chunk = messages[start:i]
        if _turn_has_agent_content(chunk):
            turns.append(chunk)
    return turns


def _turn_texts_from_messages(messages: list) -> list[str]:
    return [_pydantic_messages_to_text(t) for t in _split_messages_into_turns(messages)]
...
class ShortTermMemory:
    ...
    async def ingest_after_turn(
        ...
        async with self._write_lock:
            turn_texts = _turn_texts_from_messages(messages)
            state = await asyncio.to_thread(self._load_stm_state_sync)
            sources = state.setdefault("sources", {})
