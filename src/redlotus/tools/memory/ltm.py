...
from redlotus.tools.memory.consolidation import (
    long_term_memory_consolidation_params,
    long_term_memory_debounce_params,
    merge_looks_like_unrelated_rewrite,
)
from redlotus.tools.memory.hygiene import should_consolidate, _parse_iso
from redlotus.tools.memory.message_text import user_prompts_to_text
...
class LongTermMemory:
    ...
    _MEMORY_DIR = paths.memory_dir()                             # 可写：SOUL/USER/consolidation_state
    _TEMPLATE_DIR = paths.prompts_dir() / "LongTermMemory"       # 只读：随包提示词模板
    ...
    def _sync_read_guidance(self) -> str:
        path = self._TEMPLATE_DIR / self._GUIDANCE_FILE
    ...
    def _load_consolidation_template_sync(self) -> str:
        path = self._TEMPLATE_DIR / self._CONSOLIDATION_TEMPLATE_FILE
        if not path.is_file():
            return ""
