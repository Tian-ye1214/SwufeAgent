...
from redlotus.RAG.RAG import RAG as RAGEngineCls
...
from redlotus.workspace.workspace import MODEL_MESSAGES_GLOB, conversations_root, stm_log_key
from redlotus.tools.memory.message_text import TurnMemoryEntry, turn_entries_from_messages
from redlotus.tools.memory.hygiene import turn_key_hash, normalize_for_storage, apply_soft_forget
...
class ShortTermMemory:
    ...
    def _db_dir(self) -> Path:
        # 与 LanceDB 向量库同一解析口径，避免状态/向量分裂到两处。
        return Path(resolve_lancedb_dir(str(self._cfg["db_path"])))
